...
export default function LiveSession({ split, lang, isDark, onBack }: LiveSessionProps) {
  ...
  // ── Swap exercise ──
  const swapExercise = (exIdx: number, newExercise: { id: string; name: { en: string; ar: string }; primaryMuscle: import("@/lib/athlete-dashboard/types").MuscleGroupEnum; secondaryMuscles: import("@/lib/athlete-dashboard/types").MuscleGroupEnum[]; defaultSets: number; defaultReps: string }) => {
    setExerciseLogs((prev) =>
      prev.map((ex, ei) =>
        ei === exIdx
          ? {
