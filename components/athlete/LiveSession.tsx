...
import { WorkoutSplit, Language, ExerciseLog, SetLog, CardioLog, LiveSessionLogs } from "@/lib/types";
import { getExercisesByMuscle, MUSCLE_LABELS, DAYS_OF_WEEK } from "@/lib/data";
...
export default function LiveSession({ split, lang, isDark, onBack }: LiveSessionProps) {
  ...
  // ── Swap exercise ──
  const swapExercise = (exIdx: number, newExercise: { id: string; name: { en: string; ar: string }; primaryMuscle: import("@/lib/types").MuscleGroupEnum; secondaryMuscles: import("@/lib/types").MuscleGroupEnum[]; defaultSets: number; defaultReps: string }) => {
    setExerciseLogs((prev) =>
      prev.map((ex, ei) =>
        ei === exIdx
          ? {
